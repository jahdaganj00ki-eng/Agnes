    ...
    public static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
        ...
        public ParameterizedTypeImpl(Type type, Class<?> cls, Type... typeArr) {
            ...
            for (int i6 = 0; i6 < length; i6++) {
                Objects.requireNonNull(this.typeArguments[i6]);
                GsonTypes.checkNotPrimitive(this.typeArguments[i6]);
                Type[] typeArr3 = this.typeArguments;
                typeArr3[i6] = GsonTypes.canonicalize(typeArr3[i6]);
            }
        }
        ...
        public String toString() {
            ...
            for (int i6 = 1; i6 < length; i6++) {
                sb.append(", ");
                sb.append(GsonTypes.typeToString(this.typeArguments[i6]));
            }
            sb.append(">");
            return sb.toString();
        }
        ...
        if (cls2.isInterface()) {
            ...
            for (int i6 = 0; i6 < length; i6++) {
                ...
                if (cls2.isAssignableFrom(cls3)) {
                    return getGenericSupertype(cls.getGenericInterfaces()[i6], interfaces[i6], cls2);
                }
            }
        }
        if (!cls.isInterface()) {
            ...
            if (type2 instanceof TypeVariable) {
                ...
                if (cls3.isArray()) {
                    ...
                    if (type2 instanceof ParameterizedType) {
                        ...
                        boolean zEqual = equal(typeResolve4, ownerType);
                        Type[] actualTypeArguments = type2.getActualTypeArguments();
                        int length = actualTypeArguments.length;
                        Type[] typeArr = actualTypeArguments;
                        boolean z2 = false;
                        for (int i6 = 0; i6 < length; i6++) {
                            Type typeResolve5 = resolve(type, cls, typeArr[i6], map);
                            if (!equal(typeResolve5, typeArr[i6])) {
                                if (!z2) {
                                    typeArr = (Type[]) typeArr.clone();
                                    z2 = true;
                                }
                                typeArr[i6] = typeResolve5;
                            }
                        }
                        if (!zEqual || z2) {
                            typeNewParameterizedTypeWithOwner = newParameterizedTypeWithOwner(typeResolve4, (Class) type2.getRawType(), typeArr);
                            type2 = typeNewParameterizedTypeWithOwner;
                        }
